...
`apps/desktop` 是基于 Electron 的桌面宿主，复用 `packages/ui` 的工作区界面与 `packages/core` 的 runtime/provider 能力，并通过 `renderer -> preload -> IPC -> host` 的代理链路访问真实 Provider。

当前首批能力包括：

- `desktop` runtimeMode 与共享 provider 目录接入
...
- `pnpm --filter desktop dev:renderer`：启动 renderer 开发服务
- `pnpm --filter desktop dev:host`：构建 main/preload 并启动 Electron 宿主
- `pnpm --filter desktop build`：构建 renderer 与 main/preload
- `pnpm --filter desktop test`：运行桌面单测
- `pnpm --filter desktop test:e2e`：运行桌面 e2e（使用 mock runtime）
...
- core runtime 在 `desktop` 模式下的 provider 过滤与注入
