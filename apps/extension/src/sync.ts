...
export function shouldUseMockExtensionSync(env: AppEnv): boolean {
    const explicit = readBooleanEnv(env, 'WXT_USE_MOCK_SYNC');
    if (explicit !== null) {
        return explicit;
    }

    return env.WXT_E2E === '1';
}

export interface CreateExtensionSyncStorageProviderOptions {
...
export function createExtensionSyncStorageProvider(options: CreateExtensionSyncStorageProviderOptions = {}) {
    ...
    return new SyncStorageProvider({
        ...
        transport: shouldUseMockExtensionSync(env)
