...
async function findTaskIdByTitle(request: APIRequestContext, title: string): Promise<string> {
  let matchedId: string | null = null;
  await expect.poll(async () => {
    const response = await request.post(`${contextApiBaseUrl}/get-tasks`, {
      data: {}
    });
    expect(response.ok()).toBeTruthy();
    const payload = await response.json() as { tasks: Array<{ id: string; title: string }> };
    matchedId = payload.tasks.find((task) => task.title === title)?.id ?? null;
    return matchedId;
  }).not.toBeNull();
  return matchedId!;
}
...
async function deleteTask(request: APIRequestContext, taskId: string): Promise<void> {
  const response = await request.post(`${contextApiBaseUrl}/delete-task`, {
    data: { taskId }
  });
  expect(response.ok()).toBeTruthy();
}
...
test('web2 all-tasks supports inline editing, today defaults, execution ordering, and workspace navigation', async ({ page, request }) => {
  ...
  let createdDocumentTaskId: string | null = null;
  let createdTodayTaskId: string | null = null;

  try {
    await page.goto('/#/');
    const createDocumentTaskResponse = await request.post(`${contextApiBaseUrl}/create-task`, {
      data: {
        task: {
          id: documentTaskId,
    ...
    createdDocumentTaskId = createdDocumentTask.id;
    ...
    createdTodayTaskId = await findTaskIdByTitle(request, globalTodayTaskTitle);
    ...
    if (createdTodayTaskId) {
      await deleteTask(request, createdTodayTaskId);
    }
    if (createdDocumentTaskId) {
      await deleteTask(request, createdDocumentTaskId);
    }
  }
});
