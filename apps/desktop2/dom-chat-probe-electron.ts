 ...
 *
 * 复用「与线上完全相同」的 preload（dist/main/main/<provider>-dom.preload.cjs），
 * 即 domChat/* 共享逻辑，验证的就是线上跑的注入/提取/结束检测。
 *
 * 用法：
 *   pnpm --filter desktop2 probe:dom:electron -- chatgpt "美国第二大城市是哪里"
 *   pnpm --filter desktop2 probe:dom:electron -- gemini  "印度的首都"
 *
...
import { DESKTOP_CONTROLLED_PAGE_DOM_EVENT_FROM_PAGE_CHANNEL } from './shared/controlledPageBridge';

const __dirname = dirname(fileURLToPath(import.meta.url));
const DESKTOP2_ROOT = resolve(__dirname, '..'); // 打包到 dist/ 后，repo 路径相对 desktop2 根解析
...
const SPECS: Record<Provider, ProviderSpec> = {
    chatgpt: {
        ...
        preloadOut: resolve(DESKTOP2_ROOT, 'dist/main/main/chatgpt-dom.preload.cjs'),
        preloadSrc: '../../plugins/ai-agent/src/preload/chatgptDomPreload.ts'
    },
    gemini: {
        providerId: 'gemini-dom',
        url: 'https://gemini.google.com/app',
        partition: 'persist:chatprism-gemini-dom',
        preloadOut: resolve(DESKTOP2_ROOT, 'dist/main/main/gemini-dom.preload.cjs'),
        preloadSrc: '../../plugins/ai-agent/src/preload/geminiDomPreload.ts'
    }
};
