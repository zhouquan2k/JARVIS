...
系统 MUST 提供一个运行于 Background 的代理机制，并暴露对应的 UI 替身 Provider，以绕过浏览器插件内容脚本的 CORS 限制。代理协议 MUST 支持请求关联标识（如 `requestId`/`channelId`）以隔离普通聊天、对比分析与外部历史查询等并发流。
...
- **WHEN** UI 层发送 `GET_HISTORY_LIST` 指令并包含目标历史 provider 标识
- **THEN** Background MUST 解析该请求并调用对应历史 provider 的列表查询能力
