...
### 需求：知识工作区 MUST 提供只读图片 viewer
知识工作区 MUST 通过统一的 `DocumentViewer` registry 将常见图片 MIME 类型解析为只读图片 viewer，并在主面板中显示当前图片文件。该 viewer MUST 复用 `IContextProvider.readDocument()` 返回的 `mimeType` 与 `dataBase64` 载荷，不得要求独立的图片读取接口。

#### 场景：将支持的图片 MIME 解析到 image viewer
