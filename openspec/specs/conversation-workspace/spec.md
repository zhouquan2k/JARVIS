...
- **AND** list MUST defaultscope Agent currentresolve scoped agents #### Scenario: Keep normal chat execution semantics unchanged after manual binding
- **WHEN** the userlocalconversation `conversation.agentKey`
- **THEN** the usercontinuemessage，The system MUST NOT onlyautomaticallyswitch Agent
- **AND**  MUST conversation Agent list ### Requirement: Normal chat view MUST support multimodal attachment composition
