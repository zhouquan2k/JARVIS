import { decodeBase64, decodeTextDocument, encodeTextDocument, inferDocumentMimeType, isTextDocumentMimeType } from '@plugins/ai-agent/src/internal';
import type { ContextDocument, ContextNode, IContextProvider, MessageAttachment } from '@plugins/ai-agent/src/internal';
import type { AgentToolDefinition, AgentToolExecutionContext } from './types';
import { toolOutput } from './createAgentToolExecutor';
...
function buildReadFileOutput(document: ContextDocument, path: string) {
    if (isTextDocumentMimeType(document.mimeType)) {
        return {
            path: document.path,
            mimeType: document.mimeType,
            content: decodeTextDocument(document.dataBase64)
        };
    }

    const attachment: MessageAttachment = {
        id: `read_file-${document.path}`,
        type: document.mimeType.toLowerCase().startsWith('image/') ? 'image' : 'file',
        name: documentFileName(document.path),
        mimeType: document.mimeType,
        size: decodeBase64(document.dataBase64).length,
        base64Data: document.dataBase64
    };
    ...
    return toolOutput(
        {
            path: document.path,
            mimeType: document.mimeType,
            note: 'Binary file content is provided as an attachment for you to read directly.'
        },
        [attachment]
    );
}
    ...
    return [
        ...
        },
        {
            ...
            async execute(rawArgs, context) {
                ...
                assertPathWithinScope(path, context);
                const document = await provider.readDocument(path);
                return buildReadFileOutput(document, path);
            }
        },
        {
            id: 'search_in_scope',
