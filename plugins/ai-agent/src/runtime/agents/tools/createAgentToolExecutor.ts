import type { AgentToolBinding, AgentToolCall, AgentToolResult, MessageAttachment } from '@plugins/ai-agent/src/internal';
import type { AgentToolDeclaration, AgentToolDefinition, AgentToolExecutionContext, AgentToolExecutor } from './types';

const TOOL_OUTPUT_MARKER = '__agentToolOutput';

export interface AgentToolOutput {
    [TOOL_OUTPUT_MARKER]: true;
    result: unknown;
    attachments?: MessageAttachment[];
}

export function toolOutput(result: unknown, attachments?: MessageAttachment[]): AgentToolOutput {
    return { [TOOL_OUTPUT_MARKER]: true, result, attachments };
}

function isToolOutput(value: unknown): value is AgentToolOutput {
    return !!value
        && typeof value === 'object'
        && (value as Record<string, unknown>)[TOOL_OUTPUT_MARKER] === true;
}
