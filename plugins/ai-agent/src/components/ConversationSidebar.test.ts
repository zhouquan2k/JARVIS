...
import type { Conversation, ExternalHistoryProviderEntry } from '@plugins/ai-agent/src/internal';
...
describe('ConversationSidebar', () => {
    ...
    it('shows a separate document tag while keeping the title clean', () => {
        const wrapper = mount(ConversationSidebar, {
            props: {
                collapsed: false,
                historySource: 'local',
        ...
        expect(wrapper.get('[data-testid="local-history-item"]').text()).toContain('第一条会话');
        expect(wrapper.get('[data-testid="local-history-document-tag"]').text()).toBe('docs');
    });
