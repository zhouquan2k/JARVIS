<template>
  <section class="agent-view" data-testid="agent-view">
    ...
    <DocumentEditorPane
      ...
      :insert-link-types="conversationInsertLinkTypes"
...
import { type AgentInheritanceMode, type AgentToolBinding, type ContextDocument, type ContextNode, type ResolvedAgentConfig } from '@plugins/ai-agent/src/internal';
...
const conversationInsertLinkTypes = computed<ResolvedInsertLinkType[]>(() => {
  if (props.linkableConversations.length === 0) {
    return [];
  }
  ...
  return [{
    ...
    items: props.linkableConversations.map((conversation) => ({
      id: conversation.conversationId,
      title: conversation.title,
      markdown: `[${conversation.title}](chatprism://conversation/${encodeURIComponent(conversation.conversationId)})`
    }))
  }];
});
