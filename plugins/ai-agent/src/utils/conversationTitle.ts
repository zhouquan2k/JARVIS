export function extractNodeNameFromPath(path: string | null | undefined): string | null {
    ...
    return last ? last.replace(/\.md$/i, '') || null : null;
}
...
export function resolveConversationDocumentTag(
    documentPath: string | null | undefined,
    boundNodeName: string | null | undefined
): string | null {
    const normalizedDocumentPath = documentPath?.trim();
    if (normalizedDocumentPath) {
        return extractNodeNameFromPath(normalizedDocumentPath);
    }

    const normalizedBoundNodeName = boundNodeName?.trim();
    return normalizedBoundNodeName || null;
}

export function sanitizeConversationTitle(raw: string | null | undefined, maxLength = 10): string {
...
export function buildFallbackConversationTitle(prompt: string | null | undefined, maxLength = 10): string {
