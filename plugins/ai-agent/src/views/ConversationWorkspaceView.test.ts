...
import type { Conversation, IContextProvider, WorkspaceContext } from '@plugins/ai-agent/src/internal';
...
type LooseWorkspaceContext = { nodes: WorkspaceContext['nodes']; agentConfigs?: Record<string, ResolvedAgentConfig> };

function toWorkspaceContext(input: LooseWorkspaceContext): WorkspaceContext {
    return {
        nodes: input.nodes,
...
function createWorkspaceContextProvider(input: LooseWorkspaceContext): IContextProvider {
    const taskProvider = {
    ...
    return {
        ...
        getContext: vi.fn().mockResolvedValue(toWorkspaceContext(input)),
...
function createDeferredContextProvider() {
    let resolveRaw: ((value: WorkspaceContext) => void) | null = null;
    const contextPromise = new Promise<WorkspaceContext>((resolve) => {
        resolveRaw = resolve;
    });
    const resolveContext = (value: LooseWorkspaceContext) => resolveRaw!(toWorkspaceContext(value));
    const taskProvider = {
        getTasks: vi.fn().mockResolvedValue([]),
        createTask: vi.fn(),
        updateTask: vi.fn(),
