/**
 * Gemini 历史 bridge 的可测试核心层——不依赖 Electron。
 * 负责把 Gemini DOM 抓取能力组织成受控页可调用的 request handler。
 * Electron preload / browser bridge 只应消费这里的纯能力入口。
 */
import type { GeminiContentRequest, GeminiContentResponse } from './geminiContentProtocol';
...
export const geminiHistoryScraper = createGeminiDomScraper({
    mode: 'desktop',
    debug: debugHistory
});
...
} = geminiHistoryScraper;

export async function handleGeminiHistoryRequest(request: GeminiContentRequest): Promise<GeminiContentResponse> {
    return geminiHistoryScraper.handleRequest(request);
}

export async function handleGeminiHistoryRequestForTest(request: GeminiContentRequest): Promise<GeminiContentResponse> {
    return handleGeminiHistoryRequest(request);
}
