...
export function createGeminiDomScraper(options: GeminiDomScraperOptions = {}) {
    ...
    function queryHistoryItemElements(config: GeminiHistoryRemoteConfig, query?: string): Element[] {
        ...
        const containers = Array.from(document.querySelectorAll(config.selectors.historyListContainer));
        if (containers.length > 0) {
            const containerItems = containers.flatMap((container) => Array.from(container.querySelectorAll(itemSelector)));
            if (containerItems.length > 0) {
                return containerItems;
            }
        }

        // Fall back to document-wide query when container is absent or empty
        // (handles DOM restructuring where items exist but the container selector no longer matches)
        return Array.from(document.querySelectorAll(itemSelector));
    }
    ...
    function countHistoryAnchors(config: GeminiHistoryRemoteConfig): number {
        return queryHistoryItemElements(config).length || document.querySelectorAll('a[href*="/app/"]').length;
    }

    /**
     * The new Gemini web UI keeps the side navigation collapsed by default; the recent
     * conversation links (anchors to /app/<id>) only render after the sidebar is opened
     * and the "Recent" section is expanded. This expands them so the history list can be
     * scraped. Idempotent: returns early when conversation links are already present.
     */
    async function ensureHistoryNavigationExpanded(config: GeminiHistoryRemoteConfig): Promise<void> {
        ...
        //    Idempotent: only clicks when aria-expanded === 'false'.
        const toggles = Array.from(document.querySelectorAll('[data-test-id="expandable-section-toggle"]'));
        for (const toggle of toggles) {
            const ariaLabel = (toggle.getAttribute('aria-label') ?? '').toLowerCase();
            const matchesRecent = RECENT_KEYWORDS.some((keyword) => ariaLabel.includes(keyword));
            if (!matchesRecent) {
                continue;
            }
            if (toggle.getAttribute('aria-expanded') === 'false') {
                if (clickEl(toggle)) {
                    debugHistory('history-nav-expand-recent', { ariaLabel });
                    await sleep(900);
                }
            }
        }

        // 3. Wait for the first conversation links to render.
        for (let attempt = 0; attempt < GEMINI_HISTORY_READY_MAX_ATTEMPTS; attempt += 1) {
            if (countHistoryAnchors(config) > 0) {
                break;
            }
            await sleep(GEMINI_HISTORY_READY_RETRY_DELAY_MS);
        }

        // 4. The recent list is virtualized / lazy-loaded — scroll to load the full history.
        await scrollHistoryListToEnd(config);

        debugHistory('history-nav-expanded', {
            anchorCount: countHistoryAnchors(config)
        });
    }

    function getHistoryScrollContainer(config: GeminiHistoryRemoteConfig): HTMLElement | null {
        const items = queryHistoryItemElements(config);
        const anchor = (items[items.length - 1] ?? document.querySelector('a[href*="/app/"]')) as HTMLElement | null;
        let node: HTMLElement | null = anchor?.parentElement ?? null;
        while (node) {
            const style = window.getComputedStyle(node);
            const scrollable = /(auto|scroll)/.test(style.overflowY) && node.scrollHeight > node.clientHeight + 8;
            if (scrollable) {
                return node;
            }
    ...
    async function scrollHistoryListToEnd(config: GeminiHistoryRemoteConfig): Promise<void> {
        ...
        for (let round = 0; round < MAX_SCROLL_ROUNDS; round += 1) {
            const items = queryHistoryItemElements(config);
            const currentCount = items.length || document.querySelectorAll('a[href*="/app/"]').length;

            if (currentCount === previousCount) {
                stableRounds += 1;
                // Two consecutive rounds without growth → all items loaded.
                if (stableRounds >= 2) {
                    return;
                }
            } else {
            ...
            const scrollContainer = getHistoryScrollContainer(config);
            if (scrollContainer) {
                scrollContainer.scrollTop = scrollContainer.scrollHeight;
            } else if (lastItem instanceof HTMLElement) {
                lastItem.scrollIntoView({ block: 'end' });
            } else {
                return;
            }
        ...
        debugHistory('history-nav-scroll-capped', {
            anchorCount: queryHistoryItemElements(config).length
        });
    }
    ...
    async function handleRequest(request: GeminiContentRequest): Promise<GeminiContentResponse> {
        ...
        if (request.action === 'GET_HISTORY_LIST') {
            ...
            // New Gemini UI collapses the side navigation by default, so the "Recent"
            // conversation links are not rendered until the sidebar + section are expanded.
            if (!normalizeSearchQuery(request.query)) {
                await ensureHistoryNavigationExpanded(request.config);
            }
        }
