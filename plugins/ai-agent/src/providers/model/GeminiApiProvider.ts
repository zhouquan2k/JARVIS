...
function buildGeminiAgentContents(request: AgentRunRequest) {
    ...
    request.toolExchanges?.forEach((exchange) => {
        ...
        const responseParts: GeminiRequestPart[] = [
            {
                functionResponse: {
                    id: exchange.call.id,
        ...
        (exchange.result.attachments || [])
            .filter((attachment) => !!stripDataUriPrefix(attachment.base64Data))
            .forEach((attachment) => {
                responseParts.push(buildGeminiPartFromAttachment(attachment));
            });
...
export class GeminiApiProvider implements IAgentCapableProvider {
    ...
    async generateConversationTitle(
        prompt: string,
        maxLength = GEMINI_TITLE_MAX_LENGTH
    ): Promise<string> {
        const apiKey = this.resolveApiKey();
        if (!apiKey) {
        ...
        const modelId = await this.resolveConversationTitleModelId();
        const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelId}:streamGenerateContent?alt=sse&key=${apiKey}`;
        const payload: Record<string, unknown> = {
            contents: buildGeminiContents(
                buildGeminiConversationTitlePrompt(prompt, maxLength),
                {}
            )
        };
        ...
        return normalizeGeneratedTitle(streamResult.text, maxLength);
    }
