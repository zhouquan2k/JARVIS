...
export class ChatGPTWebProvider implements IModelProvider, IExternalConversationProvider {
    ...
    private readonly lightweightModelId: string | null;
    ...
    constructor(options: ChatGPTWebProviderOptions = {}) {
        ...
        this.lightweightModelId = options.lightweightModel?.providerId === 'chatgpt-web'
            ? options.lightweightModel.modelId
            : null;
    }
    ...
    async generateConversationTitle(
        prompt: string,
        maxLength = CHATGPT_TITLE_MAX_LENGTH
    ): Promise<string> {
        await this.ensureAccessToken();
        ...
        const titleModelId = await this.resolveConversationTitleModelId(this.lightweightModelId ?? undefined);
        const deviceId = await this.getOaiDeviceId();
        const titlePrompt = buildConversationTitlePrompt(prompt, maxLength);
        ...
        return extractStandaloneTitle(fullText, maxLength);
    }
