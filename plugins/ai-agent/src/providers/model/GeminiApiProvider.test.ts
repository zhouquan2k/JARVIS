...
describe('GeminiApiProvider', () => {
    ...
    it('appends tool-result attachments as inlineData in the function-response turn', async () => {
        const fetchMock = vi.fn().mockResolvedValue(
            createGeminiSseResponse(['done'])
        );
        vi.stubGlobal('fetch', fetchMock);
                ...
                toolExchanges: [
                    {
                        ...
                        result: {
                            toolCallId: 'call-1',
                            name: 'read_file',
                            result: '{"path":"/docs/lease.pdf","mimeType":"application/pdf"}',
                            isError: false,
                            attachments: [
                                {
                                    id: 'read_file-/docs/lease.pdf',
                                    type: 'file',
                                    name: 'lease.pdf',
    ...
    it('generates a normalized conversation title with a low-cost flash model', async () => {
        ...
        const provider = new GeminiApiProvider({ apiKey: 'test-key' });
        await expect(provider.generateConversationTitle?.('请为文档归档策略写一个简短标题', 10)).resolves.toBe('文档归档策略说明');

        expect(fetchMock).toHaveBeenCalledTimes(1);
        expect(String(fetchMock.mock.calls[0]?.[0])).toContain('/models/gemini-2.0-flash:streamGenerateContent');
        const requestBody = JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body));
        expect(requestBody.contents[0]?.parts[0]?.text).toContain('生成一个尽可能短的中文会话标题');
