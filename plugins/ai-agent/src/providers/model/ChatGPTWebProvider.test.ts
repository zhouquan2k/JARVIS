...
describe('normalizeChatGPTConversationDetail', () => {
    ...
    it('generates a normalized conversation title with a dedicated low-cost model path', async () => {
        ...
        await expect(provider.generateConversationTitle?.('请帮我梳理事故时间线和修复步骤', 10)).resolves.toBe('事故时间线梳理');
        ...
        expect(requestBody.messages[0]?.content.parts[0]).toContain('生成一个尽可能短的中文会话标题');
