...
describe('SyncStorageProvider', () => {
    ...
    it('preserves updatedAt when saving a renamed conversation through the preserve API', async () => {
        const transport = new MockSyncTransport();
        const localStore = new MemoryStorageProvider();
        const provider = new SyncStorageProvider({
            localStore,
            transport,
            syncKey: 'workspace-preserve-updated-at',
            stateStore: new MemorySyncStateStore(),
            deletedConversationStore: new MemoryDeletedConversationStateStore()
        });
        ...
        await provider.saveConversationPreservingUpdatedAt?.(createConversation({
            id: 'sync-rename',
            title: 'New title',
            updatedAt: 100
        }));

        await expect(provider.getConversation('sync-rename')).resolves.toMatchObject({
            id: 'sync-rename',
            title: 'New title',
            updatedAt: 100,
