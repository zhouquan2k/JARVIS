...
- 将[docs/workspace.dsl]中的C4图作为了解全局设计的主要入口
- 对于复杂问题、反复失败问题，先做分层定位和可观测性建设，再做修复；对跨进程、跨窗口、外部站点(dom捕获）、时序相关问题，要先拆链路并补关键阶段日志或状态观测点，必要时下载完整的数据用于一次性完整的分析，避免在错误层面反复猜测试错。
- 关于 e2e和验证阶段
